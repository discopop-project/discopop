...
class ContextObject(object):
        ...
        for read in node_reads:
            ...
            for device_id in self.seen_writes_by_device:
                ...
                if device_id == reading_device_id:
                    continue
                if read.memory_region not in self.__get_seen_writes_by_device(device_id):
                    # read memory region is currently "unknown" to the device, thus is can be skipped
                    continue
                other_devices_known_writes = self.__get_seen_writes_by_device(device_id)[read.memory_region]

                is_first_data_occurrence = False
                if read.memory_region not in self.__get_seen_writes_by_device(reading_device_id):
                    # reading device does not currently "know" about the read memory region. create a new entry.
                    self.__initialize_seen_writes_by_device(reading_device_id, read.memory_region)
                    is_first_data_occurrence = True

                known_writes = self.__get_seen_writes_by_device(reading_device_id)[read.memory_region]
                ...
                for data_write in unknown_writes:
                    ...
                    if device_id != 0 and reading_device_id != 0:
                        ...
                        if data_write.memory_region not in self.__get_seen_writes_by_device(
                            experiment.get_system().get_host_device_id()
                        ):
                            self.__initialize_seen_writes_by_device(
                                experiment.get_system().get_host_device_id(), data_write.memory_region
                            )
                        if (
                            data_write
                            not in self.__get_seen_writes_by_device(experiment.get_system().get_host_device_id())[
                ...
                for update in required_updates:
                    if update.write_data_access.memory_region not in self.__get_seen_writes_by_device(
                        update.target_device_id
                    ):
                        self.__initialize_seen_writes_by_device(
                            update.target_device_id, update.write_data_access.memory_region
                        )
                    self.__add_seen_write(update.target_device_id, update.write_data_access.memory_region, update.write_data_access)
    ...
    def add_writes(self, node_writes: Set[WriteDataAccess], writing_device_id: int):
        ...
        for write in node_writes:
            ...
            if write.memory_region not in self.__get_seen_writes_by_device(writing_device_id):
                self.__initialize_seen_writes_by_device(writing_device_id, write.memory_region)
