...
def main():
    ...
    for benchmark_number, benchmark_path in benchmark_numbers_and_paths:
        ...
        evaluation_results[benchmark_number] = (is_supported_by_tool, len(identified_data_races), identified_data_races)

    print(evaluation_results)

    # write evaluation results to evaluation_results.csv
    with open("evaluation_results_dp.csv", "w+") as f:
        f.write("Benchmark;Is supported by our tool;Identified data race lines;Data race lines\n")
        sorted_keys = list(evaluation_results.keys())
        sorted_keys.sort()
        for benchmark_number in sorted_keys:
            is_supported_by_tool, data_race_count, identified_data_races = evaluation_results[benchmark_number]
