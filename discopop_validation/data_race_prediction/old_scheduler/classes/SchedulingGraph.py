...
class SchedulingGraph(object):
    ...
    def __init__(self, dim: List[int], behavior_models: List[BehaviorModel]):
        ...
        self.__add_nodes_rec((tuple(0 for _ in range(len(dim))), -1), dim.copy(), behavior_models)
    ...
    def __add_nodes_rec(self, parent_node_identifier, dim, behavior_models: List[BehaviorModel]):
        for i in range(len(dim)):
            ...
            # start recursion
            self.__add_nodes_rec(new_node_identifier, dim_copy, behavior_models)
