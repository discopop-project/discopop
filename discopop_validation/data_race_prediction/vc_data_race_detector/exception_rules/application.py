...
def apply_exception_rules(unfiltered_data_races: List[DataRace], pet, pc_graph) -> List[DataRace]:
    ...
    for data_race in unfiltered_data_races:
        ...
        if str(data_race.get_parent_suggestion_type()) == str(PragmaType.FOR):
            dr_is_valid = __check_pragma_for_exception_rules(data_race, pet, pc_graph)
        else:
