...
def create_scheduling_graph_from_behavior_models(behavior_models: List[BehaviorModel]) -> Tuple[
    SchedulingGraph, List[int]]:
    ...
    for thread_id, behavior_model in enumerate(behavior_models):
        behavior_model.schedule_elements = __convert_operation_list_to_schedule_element_list(behavior_model.operations,
                                                                                             thread_id)
...
def __convert_operation_list_to_schedule_element_list(operation_list: List[Operation], thread_id: int) -> List[
    ScheduleElement]:
