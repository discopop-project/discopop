...
def _dep_type_and_var(dep: str) -> Tuple[str, str]:
    # dep format: "<sink> <TYPE> <source>|<var>(...)"
    dep_type = dep.split(" ")[1]
...
class TestMethods(unittest.TestCase):
    ...
    def test(self) -> None:
        ...
        # false-positive check: none of the forbidden (type, variable) pairs may appear
        # in the exact dynamic execution trace. Static analysis is intentionally
        # conservative (a may-analysis) and is not checked here, to avoid failing
        # this test due to a legitimate over-approximation unrelated to the runtime
        # detector.
        print("FORBIDDEN_DEPS: \n", forbidden_dependencies_list)

        unexpected_deps: List[str] = []
        for dep in dynamic_deps:
            dep_type, var = _dep_type_and_var(dep)
            if (dep_type, var) in forbidden_dependencies_list:
