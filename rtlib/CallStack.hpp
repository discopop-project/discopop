...
struct CallStackEntry
{
    ...
    CallStackEntry(short arg_type, LID arg_lid, unsigned long arg_counter): type(arg_type), lid(arg_lid), counter(arg_counter){};
...
struct CallStack
{
    ...
    void push(CallStackEntry* cse){
        ...
        // this is an artifact of the implementation of __dp_func_entry, as it relies on the "loops" hashmap to remove duplicates upon "insert"
        if(stack.size() > 0){
    ...
    CallStackEntry* getElement(int n){
        return stack.at(n);
    }
...
/*
void compare_callstacks(CallStack* cs_1, CallStack* cs_2, std::set<LID>* intra_iteration_dependencies,
                        std::set<LID>* inter_iteration_dependencies, std::set<LID>* intra_call_dependencies,
                        std::set<LID>* inter_call_dependencies){
