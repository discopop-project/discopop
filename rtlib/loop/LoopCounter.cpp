...
void loop_counter_output() {
  ...
  std::vector<__dp::loop_info_t> loop_infos;
  loop_infos.push_back(__dp::loop_info_t()); // dummy
  std::string tmp(getenv("DOT_DISCOPOP_PROFILER"));
  tmp += "/loop_meta.txt";
  ifile.open(tmp.data());
  while (std::getline(ifile, line)) {
    __dp::loop_info_t loop_info;
  ...
  for (auto i = 1; i < lc.loop_counters_.size(); ++i) {
    __dp::loop_info_t &loop_info = loop_infos[i];
