...
class MainScreen(object):
    ...
    def __init__(self, wizard, window_frame: tk.Frame, config_dir):
        self.push_main_screen(wizard, window_frame, config_dir)

    def push_main_screen(self, wizard, window_frame: tk.Frame, config_dir):
        ...
        self.build_configurations_frame(wizard, config_dir)
    ...
    def build_configurations_frame(self, wizard, config_dir: str):
        # clear previous contents if existent
        for c in self.configuration_frame.winfo_children():
            c.destroy()
        # build configuration frame
        self.__display_execution_configurations(wizard, config_dir)
        self.__display_configuration_buttons(wizard, config_dir)

    def __create_new_execution_configuration(self, wizard, config_dir: str):
        execution_config = ExecutionConfiguration()
        execution_config.show_details_screen(wizard, self, config_dir)

    def __display_configuration_buttons(self, wizard, config_dir: str):
        button_frame = tk.Frame(self.configuration_frame)
        new_button = tk.Button(button_frame, text="New..", command=lambda: self.__create_new_execution_configuration(wizard, config_dir))
        new_button.grid(row=1, column=1, sticky="nsew")
        button_frame.pack()


    def __display_execution_configurations(self, wizard, config_dir):
        # based on https://blog.teclado.com/tkinter-scrollable-frames/
        # load configuration options
        configs: List[ExecutionConfiguration] = self.load_execution_configurations(config_dir)
        ...
        for row, config in enumerate(configs):
            button = config.get_as_button(wizard, self, scrollable_frame, config_dir)
