...
To obtain data dependences, we need to instrument the target application. Running the instrumented application will result in a text file containing all the dependences that are located in the present working directory.
	...
	./<APP_NAME>
-->
#### CU generation and dependence profiling
To obtain the computational unit (CU graph) and the data dependencies of the target application, please run the following commands. 
Firstly, the target application is compiled into LLVM-IR. The `-g -O0 -emit-llvm -fno-discard-value-names` flags are required due to the additional created metadata.
Secondly, the CUs will be extracted and the target application will be instrumented using `opt` to allow the dependence profiling.
Lastly, compiling and executing the instrumented application will result in a text file containing all identified data dependencies. The file will be located in the current working directory.
    
    clang++ -g -c -O0 -S -emit-llvm -fno-discard-value-names <TARGET>.cpp -o <TARGET>.ll
    opt -S -load=<PATH_TO_DISCOPOP_BUILD_DIR>/libi/LLVMDiscoPoP.so --DiscoPoP <TARGET>.ll -o <TARGET>_dp.ll --fm-path FileMapping.txt
    clang++ <TARGET>_dp.ll -o out_prof -L<PATH_TO_DISCOPOP_BUILD_DIR>/rtlib -lDiscoPoP_RT -lpthread
    ./out_prof
...
To obtain the list of reduction operations in the target application, we need to instrument the target application. Running the instrumented application will result in a text file containing all the reductions that are located in the present working directory.
	...
	opt -S -load=<PATH_TO_DISCOPOP_BUILD_DIR>/libi/LLVMDPReduction.so --DPReduction <TARGET>.ll -o <TARGET>_dp_red.ll --fm-path FileMapping.txt
	clang++ <TARGET>_dp_red.ll -o out_red -L<PATH_TO_DISCOPOP_BUILD_DIR>/rtlib -lDiscoPoP_RT -lpthread
	./out_red
